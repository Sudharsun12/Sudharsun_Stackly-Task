...
app.config['MAX_CONTENT_LENGTH'] = MAX_FILE_SIZE  # Flask auto-rejects files > 2MB with 413

def allowed_file(filename):
    # Must have a dot AND extension must be in the allowed set
...
def get_products():
    ...
    # ── Query 1: COUNT total matching products ───────────────────────────
    ...
    # ── Query 2: Fetch only the current page ─────────────────────────────
    # OFFSET = (page - 1) * limit
    #   page 1 → OFFSET 0   (rows 1–8)
    #   page 2 → OFFSET 8   (rows 9–16)
    #   page 3 → OFFSET 16  (rows 17–24)
    page_query = f'''SELECT p.*, c.name AS category_name
                     FROM products p
                     LEFT JOIN categories c ON p.category_id = c.id
                     {where}
                     {order}
...
def all_orders():
    ...
    # ── Query 2: Fetch only current page of orders ───────────────────────
...
def upload_image():
    ...
    # Check that a file was actually sent in the request
    if 'image' not in request.files:
        return jsonify({"error": "No file provided"}), 400

    file = request.files['image']

    # Empty filename means the user submitted the form without selecting a file
    if file.filename == '':
        return jsonify({"error": "No file selected"}), 400

    # Reject files that aren't images (e.g. .txt, .pdf, .exe)
    if not allowed_file(file.filename):
        return jsonify({"error": "Invalid file type. Allowed: png, jpg, jpeg, webp"}), 400

    # Generate a unique filename using uuid4 so two uploads of "photo.jpg"
    # never overwrite each other — every filename is globally unique
    ext         = file.filename.rsplit('.', 1)[1].lower()
    unique_name = f"{uuid.uuid4().hex}.{ext}"
    filepath    = os.path.join(app.config['UPLOAD_FOLDER'], unique_name)

    # Save the file to disk inside static/uploads/
    file.save(filepath)

    # Return the URL path the frontend will use to display the image
    # Flask auto-serves everything inside /static/ — no extra route needed
